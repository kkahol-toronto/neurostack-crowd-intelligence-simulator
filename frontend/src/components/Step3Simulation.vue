          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- Fallback: unknown type with content -->
...
const props = defineProps({
  ...
  minutesPerRound: {
    type: Number,
    default: 30 // Simulated minutes per round
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const runStatus = ref({})
const allActions = ref([]) // All actions (append-only)
const actionIds = ref(new Set()) // Dedupe by action id
const scrollContainer = ref(null)

// Computed
// Chronological feed (oldest → newest, bottom = latest)
...
// Simulated elapsed time from round count × minutes per round
...
// Last seen round per platform (for log lines on change)
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Log when each platform advances a round
      ...
      // Also treat as done when each enabled platform reports completed
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ All enabled platforms finished')
        }
...
const checkPlatformsCompleted = (data) => {
  ...
  // Platform “enabled” if it has actions, was running, or already completed
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  if (!twitterEnabled && !redditEnabled) return false
  
  // Every enabled platform must be completed
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // No auto-scroll; new items append at bottom
    }
  } catch (err) {
...
const handleNextStep = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✗ Report generation failed to start: ${res.error || 'Unknown error'}`)
