<template>
  ...
  >
    <!-- Background: tech grid (when projects exist or loading) -->
    ...
    <!-- Card stack (when projects exist) -->
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <!-- Description (truncated requirement) -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 (run simulation) and Step 5 (deep interaction) must be started during a live run; they are not available from history playback.</span>
            </div>
          </div>
        </div>
      </Transition>
...
let observer = null
let isAnimating = false  // Animation lock to avoid flicker
let expandDebounceTimer = null  // Debounce expand/collapse
let pendingState = null  // Pending target expand state
...
// Container min-height from card count
const containerStyle = computed(() => {
  ...
  // rows * card height + gaps + small bottom padding
  const expandedHeight = rows * CARD_HEIGHT + (rows - 1) * CARD_GAP + 10
  
  return { minHeight: `${expandedHeight}px` }
})

// Per-card transform for fan / grid layout
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Cards in this row for centering
...
// Short simulation id for display
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Always record latest desired expand state
        pendingState = shouldExpand
        
        // Reset debounce when scroll intent changes
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // During animation, defer until unlock
        ...
        // Debounce: shorter expand (50ms), longer collapse (200ms)
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Unlock then apply any newer pending state
          setTimeout(() => {
            isAnimating = false
            
            // Re-run pending after animation
    ...
    },
    {
      ...
      // Shrink bottom inset so expand needs more scroll into view
...
onMounted(async () => {
  ...
  // Init observer after layout
  setTimeout(() => {
    initObserver()
  }, 100)
})

// keep-alive: refresh on activate
onActivated(() => {
  loadHistory()
})
...
.grid-pattern {
  ...
  /* Anchor top-left so height growth extends downward */
...
/* Per-feature accent when available */
