...
const toggleRawResult = (timestamp, event) => {
  // Remember button position in the viewport before toggling
  ...
  // After DOM update, adjust scroll so the button stays in the same visual place
...
/** Backend / LLM may emit these when a platform has no reply (Chinese variants via escapes to avoid scattering literals). */
const isNoReplyPlaceholderText = (text) => {
  if (!text) return true
  const t = text.trim()
  return (
    t === '\uff08\u8be5\u5e73\u53f0\u672a\u83b7\u5f97\u56de\u590d\uff09' ||
...
const parseInsightForge = (text) => {
  ...
  try {
    // Matches zep_tools.InsightForgeResult.to_text() (English)
    const queryMatch = text.match(/Analytical question:\s*(.+?)(?:\n|$)/i)
    ...
    const relMatch = text.match(/Relation chains:\s*(\d+)/i)
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const activeSection = text.match(/### Currently valid facts[^\n]*\n([\s\S]*?)(?=\n###|$)/i)
    if (activeSection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/\*\*Agents interviewed:\*\*\s*(\d+)\s*\/\s*(\d+)/i)
    if (countMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (name) {
          ...
          currentName &&
          line.trim() &&
          !line.match(/^Not selected|^In summary|^Final selection/i)
        ) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        const twitterMatch = answerText.match(
          /Twitter (?:platform )?answer[:：]?\s*\n?([\s\S]*?)(?=Reddit (?:platform )?answer[:：]?|$)/i
        )
        const redditMatch = answerText.match(
          /Reddit (?:platform )?answer[:：]?\s*\n?([\s\S]*?)$/i
        )
        ...
        if (!twitterMatch && redditMatch) {
          if (interview.redditAnswer && !isNoReplyPlaceholderText(interview.redditAnswer)) {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && !isNoReplyPlaceholderText(interview.twitterAnswer)) {
    ...
    const summaryMatch = text.match(/### Summary and key takeaways\n([\s\S]*?)$/i)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/Found\s+(\d+)\s+relevant items/i)
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = isNoReplyPlaceholderText
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* muted gray, not status-tinted */
  font-weight: 500;
}
