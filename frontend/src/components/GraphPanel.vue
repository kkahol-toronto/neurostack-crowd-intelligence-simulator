    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">A few items are still processing; try refreshing the graph shortly.</span>
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transitioned from simulating to idle — show finished hint
...
const renderGraph = () => {
  ...
  // Edge data: counts per node pair and self-loops by node
  const edgePairCount = {}
  ...
  // Index of current edge within each node pair
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // Self-loop nodes already merged
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Full detail for each self-loop
        }
      })
      return
    }
    ...
    // Whether edge direction differs from canonical (source UUID < target UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Curvature: spread multiple edges; straight when single
    let curvature = 0
    if (totalCount > 1) {
      // Even curvature spread; range grows with edge count
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // Flip curvature when direction is reversed so bundles stay separated
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Force simulation — link distance scales with parallel edge count
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Base distance 150; +50 per extra parallel edge
    ...
    // Weak center pull so detached groups drift toward middle
  ...
  const getLinkPath = (d) => {
    ...
    // Quadratic control point — offset scales with distance and parallel count
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Perpendicular offset; larger when more parallel edges
  ...
  const getLinkMidpoint = (d) => {
    ...
    if (d.isSelfLoop) {
      // Self-loop label to the right of node
      return { x: sx + 70, y: sy }
    }
    ...
    // B(t) at t=0.5 for quadratic Bezier
  ...
  const link = linkGroup.selectAll('path')
    ...
    .on('click', (event, d) => {
      ...
      // Reset previous edge selection style
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Record position only; distinguish click vs drag
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        ...
        if (!d._isDragging && distance > 3) {
          // Restart simulation on first real drag
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Cool down simulation only after a real drag
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    // Edge labels: horizontal, no rotation
    linkLabels.each(function(d) {
      const mid = getLinkMidpoint(d)
      d3.select(this)
        .attr('x', mid.x)
        .attr('y', mid.y)
        .attr('transform', '') // Keep horizontal
    })
