    ...
    <div class="scroll-container">
        ...
        <div class="card-content">
          ...
          <div v-if="simulationId" class="info-card">
            ...
            <div class="info-row">
              ...
              <span class="info-value mono">{{ taskId || 'Async task finished' }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            The LLM sets world clock speed, feed algorithms, per-agent active hours, posting rates, and event triggers from the scenario and seeds.
          </p>
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-grid">
                <div class="config-item">
                  <span class="config-item-label">Simulation span</span>
            ...
            <div class="config-block">
              <div class="config-block-header">
                <span class="config-block-title">Feed / ranking config</span>
              </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            From the narrative direction, generate seed events and hot topics to set the world’s initial state.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Round settings (shown after config and round count exist) -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation rounds</span>
                <span class="section-desc">NEUROSTACK_CROWD_INTELLIGENCE_SIMULATOR plans roughly <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> h of real time; each round is <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> min of simulated time.</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>If ~100 agents: about {{ Math.round(customMaxRounds * 0.6) }} min</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        If ~100 agents: about {{ Math.round(autoGeneratedRounds * 0.6) }} min
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">First run? Switch to custom mode with fewer rounds for a quicker preview and fewer errors ➝</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            <div class="info-item">
              <span class="info-label">Stated age</span>
              <span class="info-value">{{ selectedProfile.age || '-' }} yrs</span>
            </div>
            <div class="info-item">
              <span class="info-label">Stated gender</span>
            ...
            <div class="info-item">
              <span class="info-label">Stated MBTI</span>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              ...
              <div class="dimension-card">
                ...
                <span class="dim-desc">Learned patterns and style</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">Distinct memories</span>
                <span class="dim-desc">Memories grounded in seeds</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">Social graph</span>
                <span class="dim-desc">Links and interaction map</span>
              </div>
            </div>
...
// State
const phase = ref(0) // 0 create instance, 1 personas, 2 config, 3 orchestration, 4 ready
...
// Custom max rounds (optional)
const useCustomRounds = ref(false) // Default: auto round count
const customMaxRounds = ref(40)   // Suggested default

/** Map progress message label to API stage key when progress_detail is absent */
function labelToStageKey(label) {
  if (!label) return ''
  const s = label.toLowerCase()
  if (s.includes('reading')) return 'reading'
  if (s.includes('profile')) return 'generating_profiles'
  if (s.includes('configuration') || s.includes('simulation config')) return 'generating_config'
  if (s.includes('script')) return 'copying_scripts'
  return ''
}

// Watch API stage keys (see simulation.py progress_callback)
...
// Auto round count from time_config (no hard-coded default duration)
const autoGeneratedRounds = computed(() => {
  ...
  return Math.max(calculatedRounds, 40) // Keep slider range sane
})
...
// Resolve display username by agent index
...
// Total interested_topics across personas
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    addLog(`Starting simulation with custom max rounds: ${customMaxRounds.value}`)
  } else {
    addLog(`Starting simulation with auto max rounds: ${autoGeneratedRounds.value}`)
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.progress_detail) {
        ...
        if (match) {
          currentStage.value = labelToStageKey(match[3].trim())
        }
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Only overwrite expected total when API sends a value
      if (res.data.total_expected) {
        expectedTotal.value = res.data.total_expected
      }
      
      // Entity types present in profiles
      const types = new Set()
      ...
      // Log profile progress when count changes
