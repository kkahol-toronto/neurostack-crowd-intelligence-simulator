<template>
  <div class="process-page">
    <!-- Top bar -->
    <nav class="navbar">
      <div class="nav-brand" @click="goHome">NEUROSTACK_CROWD_INTELLIGENCE_SIMULATOR</div>
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">Graph build starts automatically when ontology is ready</p>
          </div>
          
          <!-- Building, no nodes yet -->
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After upload, the LLM analyzes the document and generates an ontology for simulation (entity and relation types).
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">How it works</div>
                <div class="detail-content">
                  Using the ontology, documents are chunked and the Zep API builds the graph and extracts entities and relations.
                </div>
              </div>
...
// Project id (may change from 'new')
...
const currentPhase = ref(-1) // -1 upload · 0 ontology · 1 graph · 2 done
...
const goToNextStep = () => {
  // TODO: wire navigation to environment setup
  alert('Environment setup is not wired from this view yet.')
}
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'No pending files — go back to home'
    loading.value = false
    return
  }
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        buildProgress.value = {
          progress: 100,
          message: 'Complete — loading graph...'
        }
        ...
        if (projectResponse.success) {
          ...
          if (projectResponse.data.graph_id) {
            console.log('📊 Full graph:', projectResponse.data.graph_id)
...
const renderGraph = () => {
  ...
  // Edges (thin visible + wide hit target)
  ...
  simulation.on('tick', () => {
    // Edge positions (stroke + hit area)
...
.nav-brand {
  font-size: clamp(0.42rem, 0.55vw + 0.32rem, 0.68rem);
  font-weight: 700;
