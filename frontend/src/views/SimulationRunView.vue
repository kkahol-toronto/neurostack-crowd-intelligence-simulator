  ...
  <div class="main-view">
    ...
    <header class="app-header">
      <div class="header-left">
        <div class="brand" @click="router.push('/')">NEUROSTACK_CROWD_INTELLIGENCE_SIMULATOR</div>
      </div>
...
// maxRounds from query so child mounts with correct value
const maxRounds = ref(route.query.maxRounds ? parseInt(route.query.maxRounds) : null)
const minutesPerRound = ref(30) // minutes per round (default 30)
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        addLog(`Close env failed — force stopping...`)
        try {
...
const handleNextStep = () => {
  addLog('Enter Step 4: report (handled by Step3Simulation)')
}

// --- Data Logic ---
const loadSimulationData = async () => {
  try {
    addLog(`Load simulation: ${currentSimulationId.value}`)
    ...
    if (simRes.success && simRes.data) {
      ...
      try {
        ...
        addLog(`Time config fallback: ${minutesPerRound.value} min/round`)
      }
