...
and returns full lists to callers.
"""
...
logger = get_logger('neurostack_cis.zep_paging')
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Single page fetch with exponential backoff; retries transient network/IO errors only."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Fetch all graph nodes up to max_items (default 2000); each page uses retry."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Fetch all edges for a graph; each page uses retry."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
