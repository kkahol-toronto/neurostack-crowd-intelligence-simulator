...
logger = get_logger('neurostack_cis.api.report')
...
def generate_report():
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "The map ID is missing, please make sure the map has been built"
            }), 400
        ...
        # Generate report_id in advance so that it can be returned to the front end immediately
        import uuid
...
def get_generate_status():
    ...
    try:
        ...
        # If simulation_id is provided, first check whether there is a completed report
        if simulation_id:
            existing_report = ReportManager.get_report_by_simulation(simulation_id)
            if existing_report and existing_report.status == ReportStatus.COMPLETED:
                return jsonify({
...
def get_report(report_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to obtain report: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_report_by_simulation(simulation_id: str):
    ...
    try:
        ...
        if not report:
            return jsonify({
                "success": False,
                "error": f"There is no report for this simulation: {simulation_id}",
                "has_report": False
            }), 404
...
def get_report_progress(report_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to obtain report progress: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_report_sections(report_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to obtain chapter list: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def check_report_status(simulation_id: str):
    """
    Check if the simulation has reports, and the status of the reports
    ...
    try:
        ...
        # Unlock the interview only after the report is completed
