...
logger = get_logger('neurostack_cis.api.simulation')
...
def get_graph_entities(graph_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to obtain map entity: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
...
def get_entity_detail(graph_id: str, entity_uuid: str):
    ...
    try:
        ...
        logger.error(f"Failed to obtain entity details: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_entities_by_type(graph_id: str, entity_type: str):
    ...
    try:
        ...
        logger.error(f"Failed to obtain entity: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
    ...
    Request (JSON):
        {
            ...
            "graph_id": "neurostack_xxxx", // Optional, if not provided, it will be obtained from the project
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    if not os.path.exists(simulation_dir):
        return False, {"reason": "The simulation directory does not exist"}
    
    # Necessary file list (excluding scripts, scripts are located in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # The following status indicates that the preparation work has been completed:
        # - ready: ready to run
        ...
        if status in prepared_statuses and config_generated:
            ...
            # If the status is preparing but the file has been completed, automatically update the status to ready
            if status == "preparing":
                try:
                    ...
                    logger.warning(f"Automatic update status failed: {e}")
...
def prepare_simulation():
    ...
    try:
        ...
        # Check whether preparation is complete (to avoid repeated generation)
        if not force_regenerate:
            logger.debug(f"Check whether simulation {simulation_id} is ready to complete...")
        ...
        # ========== Synchronously obtain the number of entities (before starting the background task) ==========
        # In this way, the front end can obtain the expected total number of Agents immediately after calling prepare.
        try:
            ...
            filtered_preview = reader.filter_defined_entities(
                ...
                enrich_with_edges=False # Do not obtain edge information and speed up
            )
        ...
        # Update simulation status (including the number of entities obtained in advance)
        ...
        def run_prepare():
            try:
                ...
                #Storage stage progress details
                stage_details = {}
                ...
                logger.error(f"Failed to prepare for simulation: {str(e)}")
                task_manager.fail_task(task_id, str(e))
                
                #Update simulation status to failure
                state = manager.get_simulation(simulation_id)
                if state:
        ...
        return jsonify({
            ...
            "data": {
                ...
                "expected_entities_count": state.entities_count, # The expected total number of Agents
...
def get_prepare_status():
    ...
    try:
            ...
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "Preparations have been completed",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
        ...
        if not task:
            # The task does not exist, but if there is a simulation_id, check whether it is ready to be completed.
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
                        ...
                        "data": {
                            ...
                            "status": "ready",
                            "progress": 100,
                            "message": "The task has been completed (preparation work already exists)",
                            "already_prepared": True,
                            "prepare_info": prepare_info
                        }
                    })
...
def get_simulation(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to obtain simulation status: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    try:
        ...
        # Sort by creation time in reverse order and return the latest
    ...
    try:
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # Use the total_rounds set by the user, if not, use the recommended number of rounds
            ...
            # Get the associated report_id (find the latest report of this simulation)
...
def get_simulation_config(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to obtain configuration: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def generate_profiles():
    ...
    Request (JSON):
        {
            "graph_id": "neurostack_xxxx", // required
...
def start_simulation():
    ...
    try:
        ...
        # Intelligent processing status: if preparation is completed, restart is allowed
        if state.status != SimulationStatus.READY:
            # Check if the preparation work has been completed
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)

            if is_prepared:
                # Preparation work has been completed, check whether there are running processes
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            try:
                                ...
                                logger.warning(f"A warning occurred when stopping the simulation: {str(e)}")
                        else:
                            return jsonify({
                                "success": False,
                ...
                if force:
                    ...
                    if not cleanup_result.get("success"):
                        logger.warning(f"A warning occurred when cleaning the log: {cleanup_result.get('errors')}")
                    force_restarted = True

                # The process does not exist or has ended, reset the status to ready
                logger.info(f"Simulation {simulation_id} preparations have been completed, and the reset status is ready (original status: {state.status.value})")
                state.status = SimulationStatus.READY
                manager._save_simulation_state(state)
            else:
                # Preparation work is not completed
                return jsonify({
                    "success": False,
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "A valid graph_id is required to enable graph memory update. Please ensure that the project has built a graph"
                }), 400
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to obtain detailed status: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_timeline(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to obtain timeline: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
        ...
        if not os.path.exists(db_path):
            return jsonify({
                ...
                "data": {
                    ...
                    "count": 0,
                    "posts": [],
                    "message": "The database does not exist, the simulation may not have been run yet"
                }
            })
...
def interview_agent():
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "The simulation environment is not running or has been shut down. Please ensure that the simulation has completed and entered wait for command mode."
            }), 400
        
        # Optimize prompt and add prefix to avoid Agent calling tools
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            if 'agent_id' not in interview:
                return jsonify({
                    "success": False,
                    "error": f"Item {i+1} of the interview list is missing agent_id"
                }), 400
            if 'prompt' not in interview:
                return jsonify({
                    "success": False,
                    "error": f"Item {i+1} of the interview list is missing prompt"
                }), 400
            ...
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify({
                    "success": False,
                    "error": f"The platform of item {i+1} in the interview list can only be 'twitter' or 'reddit'"
                }), 400
...
def get_env_status():
    ...
    try:
        ...
        logger.error(f"Failed to obtain environment status: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
