...
# Get the logger
logger = get_logger('neurostack_cis.api')
...
# ============== Interface 1: Upload files and generate the body ==============
...
def build_graph():
    ...
    try:
        ...
        if project.status == ProjectStatus.CREATED:
            return jsonify({
                "success": False,
                "error": "The project has not yet generated ontology, please call /ontology/generate first"
            }), 400
        
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "The graph is under construction, please do not submit again. If you need to force reconstruction, please add force: true",
                "task_id": project.graph_build_task_id
            }), 400
        
        # If forced to rebuild, reset status
        ...
        # Get configuration
        graph_name = data.get('graph_name', project.name or 'NeuroStack Crowd Intelligence Simulator Graph')
        ...
        logger.info(f"Create graph construction task: task_id={task_id}, project_id={project_id}")
        ...
        def build_task():
            build_logger = get_logger('neurostack_cis.build')
            try:
                build_logger.info(f"[{task_id}] starts building the graph...")
                task_manager.update_task(
                    task_id, 
                    status=TaskStatus.PROCESSING,
                    message="Initialize graph construction service..."
                )
