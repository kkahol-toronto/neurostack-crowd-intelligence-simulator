...
Features:
- After the simulation finishes, keep the environment alive and enter command-wait mode
...
Usage:
    ...
    python run_twitter_simulation.py --config /path/to/simulation_config.json --no-wait  # exit immediately when done
"""
...
class UnicodeFormatter(logging.Formatter):
    """Formatter that turns Unicode escape sequences into readable characters."""
...
def setup_oasis_logging(log_dir: str):
    """Configure OASIS loggers with fixed log file names."""
...
class IPCHandler:
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict]) -> bool:
        ...
        try:
            actions = {}
            agent_prompts = {}  # prompt per agent
...
class TwitterSimulationRunner:
    ...
    def __init__(self, config_path: str, wait_for_commands: bool = True):
        ...
        Args:
            ...
            wait_for_commands: After simulation, wait for IPC commands (default True)
        """
    ...
    def _get_profile_path(self) -> str:
        """Profile CSV path (OASIS Twitter)."""
    ...
    def _create_model(self):
        ...
        # Fallback to config if .env has no model name
        if not llm_model:
    ...
    def _get_active_agents_for_round(
        ...
        """
        Choose which agents are active this round from time config.
...
async def main():
    ...
    # Logging: fixed filenames, clear old logs
...
def setup_signal_handlers():
    """
    Handle SIGTERM/SIGINT so the process can shut down cleanly
    (close DB, environment, etc.).
    """
